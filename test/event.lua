...
function m.test_connect()
    ...
    cep:event_init(cfd:handle(), cfd, epoll.EPOLLIN | epoll.EPOLLOUT)
...
function m.test_send_recv()
    ...
    sep:event_init(newfd:handle(), newfd, epoll.EPOLLIN | epoll.EPOLLOUT)
...
function m.test_shutdown()
    ...
    cep:event_init(cfd:handle(), cfd, epoll.EPOLLIN | epoll.EPOLLOUT | epoll.EPOLLRDHUP)
    assertEpollWait(sep, {sfd, "EPOLLIN"})
    assertEpollWait(cep, {cfd, "EPOLLOUT"})
    local newfd <close> = sfd:accept()
    sep:event_init(newfd:handle(), newfd, epoll.EPOLLIN | epoll.EPOLLOUT | epoll.EPOLLRDHUP)
